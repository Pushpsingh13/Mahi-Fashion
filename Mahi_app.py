...
# Configuration / files
# -------------------------
MENU_EXCEL = "Product_Details_Updated_New.xlsx"
...
def create_sample_menu() -> pd.DataFrame:
    """Create a small sample menu DataFrame (used to initialize or restore)."""
    return pd.DataFrame({
        "Item": ["Kurti", "Saree", "Lehenga"],
        "Size": ["L-XL", "Free", "M,L,XL"],
        "Price": [615, 1200, 2500],
        "Images": ["", "", ""],
        # you can add more Image columns if needed
    })

def ensure_menu_exists():
    """Ensure the Excel exists; if not, create a sample."""
...
def find_image_columns(df: pd.DataFrame) -> List[str]:
    """Return list of columns that look like image columns (Image, Images, Image1, image2...)."""
    return [c for c in df.columns if c.lower().startswith("image")]

def load_menu() -> pd.DataFrame:
    """Load products from the Excel into a DataFrame; tolerate missing columns."""
    ensure_menu_exists()
    ...
    # ensure image columns exist (we'll detect dynamically)
    image_cols = find_image_columns(df)
    # create a unified list column All_Images
    if image_cols:
...
def write_menu(df: pd.DataFrame):
    """Write DataFrame back to Excel. Keep existing non-image columns and write image columns if present."""
    ...
    if "Price" not in df.columns:
        df["Price"] = 0.0
    # Expand All_Images into Image, Image2... columns
    max_imgs = df.get("All_Images", pd.Series([[]]*len(df))).map(len).max() if "All_Images" in df.columns else 0
    max_imgs = int(max_imgs or 0)
    # create image columns
    for i in range(max(1, max_imgs)):
        col = "Images" if i == 0 else f"Images{i+1}"
        def _get_img_cell(imgs, idx=i):
            try:
                return imgs[idx]
            except Exception:
                return ""
        if "All_Images" in df.columns:
            df[col] = df["All_Images"].apply(lambda imgs, idx=i: _get_img_cell(imgs, idx))
        else:
            if col not in df.columns:
                df[col] = ""
    # Drop helper column if present (we will keep it when saving to avoid losing data)
    # Save to Excel
    df_to_save = df.copy()
    # Optionally drop internal-only columns if you want
    if "All_Images" in df_to_save.columns:
        # keep All_Images transformed into Images* columns; remove All_Images to keep Excel clean
...
with tab_cart:
    ...
    if st.session_state.last_checkout:
        ...
        for i, item in enumerate(st.session_state.cart):
            ...
            with col1:
                # Find the product in the menu_df to get the image
                product_details_list = menu_df[menu_df['Item'] == item['item']]
                if not product_details_list.empty:
                    product_details = product_details_list.iloc[0]
                    images = product_details.get('All_Images', [])
                    if images:
                        st.image(images[0], width=100)
                else:
                    st.image("https://via.placeholder.com/100", width=100) # Placeholder
            with col2:
    ...
    if pw == ADMIN_PASSWORD:
        ...
        with st.form("add_product_form"):
            ...
            if add_btn:
                ...
                if len(menu_df) > 0:
                    menu_df = pd.concat([menu_df, pd.DataFrame([row])], ignore_index=True)
                else:
                    menu_df = pd.DataFrame([row])
